...
const obras = ({ totalPages, errorPosts, dataPosts }) => {
												...
												<figure className="snip0015 large">
													{obra._embedded['wp:featuredmedia'] ? (
														<img
															...
															src={
																obra._embedded['wp:featuredmedia'][0].source_url
															}
							...
							<div className="media__buttons">
								{query.page !== '1' && (
									<Link href={`/obras/${parseInt(query.page) - 1}`}>
										<a className="media__buttons__button -black">Anterior</a>
									</Link>
								)}
								{totalPages !== query.page && (
									<Link href={`/obras/${parseInt(query.page) + 1}`}>
										<a className="media__buttons__button -yellow">Siguiente</a>
									</Link>
								)}
							</div>
						</>
...
export async function getServerSideProps({ params }) {
	try {
		const res = await fetch(
			`${process.env.API_URL}/wp-json/wp/v2/posts?per_page=4&_embed&page=${params.page}&categories=4`
		)
